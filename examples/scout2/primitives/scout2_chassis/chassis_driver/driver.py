...
"""Scout2 chassis primitive — Capability-based driver.

Owns `robonix/primitive/chassis/*`. Publishes /cmd_vel from the `move` rpc.
Subscribes to /odom for odometry. Scrapes `scout_status` for health info.
...
def move(req: "chassis_pb2.ExecuteMoveCommand_Request") -> "chassis_pb2.ExecuteMoveCommand_Response":
    ...
    msg = req.command
    speed_mps = float(os.environ.get("SCOUT2_CHASSIS_SPEED_MPS", "0.3"))
    ang_speed_rps = float(os.environ.get("SCOUT2_CHASSIS_ANG_SPEED_RPS", "0.6"))
    default_dur = float(os.environ.get("SCOUT2_CHASSIS_CMD_DURATION_SEC", "1.0"))
...
# ── lifecycle ────────────────────────────────────────────────────────────────
@scout2_chassis.on_init
def init(cfg):
    global cmd_vel_pub
    odom_topic = cfg.get("odom_topic") or os.environ.get("SCOUT2_ODOM_TOPIC", "/odom")
    twist_in_topic = cfg.get("cmd_vel_topic") or os.environ.get("SCOUT2_CMD_VEL_TOPIC", "/cmd_vel")

    from geometry_msgs.msg import Twist  # type: ignore
    cmd_vel_pub = scout2_chassis.create_publisher(
        "robonix/primitive/chassis/twist_in",
        topic=twist_in_topic, msg_type=Twist, qos="best_effort",
        declare=False,
    )
    scout2_chassis.declare_ros2_topic("robonix/primitive/chassis/twist_in", twist_in_topic, qos="best_effort")
    scout2_chassis.declare_ros2_topic("robonix/primitive/chassis/odom",     odom_topic,     qos="best_effort")
    return Ok()


@scout2_chassis.on_shutdown
def shutdown():
    global cmd_vel_pub
    if cmd_vel_pub is not None:
        try:
            ...
            print(f"[scout2_chassis] shutdown zero Twist failed: {exc}", flush=True)
        finally:
            cmd_vel_pub = None
    return Ok()
