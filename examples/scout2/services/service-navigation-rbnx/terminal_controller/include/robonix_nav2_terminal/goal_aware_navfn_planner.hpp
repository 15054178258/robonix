...
private:
  /**
   * Find nearest reachable goal point within 2m radius using spiral search.
   * Returns std::nullopt if no reachable point found.
   */
  std::optional<geometry_msgs::msg::PoseStamped>
  findNearestReachableGoal(const geometry_msgs::msg::PoseStamped &goal);

  /**
   * Create plan to a reachable goal point.
   */
  nav_msgs::msg::Path
  createPlanToReachableGoal(const geometry_msgs::msg::PoseStamped &start,
