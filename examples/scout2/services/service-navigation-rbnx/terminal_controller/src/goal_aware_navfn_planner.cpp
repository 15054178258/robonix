...
GoalAwareNavfnPlanner::createPlan(const geometry_msgs::msg::PoseStamped &start,
  ...
  // If goal is on lethal obstacle, try to find nearest reachable point
  if (goal_cost >= nav2_costmap_2d::INSCRIBED_INFLATED_OBSTACLE) {
    RCLCPP_WARN(node_->get_logger(),
                "%s goal (%.2f, %.2f) is on obstacle (cost=%u), searching "
                "for nearest reachable point",
                name_.c_str(), goal.pose.position.x, goal.pose.position.y,
                static_cast<unsigned int>(goal_cost));

    auto adjusted_goal = findNearestReachableGoal(goal);
    if (adjusted_goal) {
      RCLCPP_INFO(node_->get_logger(),
                  "%s adjusted goal to (%.2f, %.2f)",
                  name_.c_str(), adjusted_goal->pose.position.x,
                  adjusted_goal->pose.position.y);
      // Try to plan to the adjusted goal
      return createPlanToReachableGoal(start, *adjusted_goal);
    } else {
      RCLCPP_ERROR(node_->get_logger(),
                   "%s no reachable point found near goal (%.2f, %.2f)",
                   name_.c_str(), goal.pose.position.x, goal.pose.position.y);
      return nav_msgs::msg::Path();
    }
  }
...
GoalAwareNavfnPlanner::findNearestReachableGoal(
    const geometry_msgs::msg::PoseStamped &goal) {
  const double resolution = costmap_->getResolution();
  const int max_search_cells = static_cast<int>(2.0 / resolution);  // 2m radius

  // Spiral search pattern
  for (int radius = 1; radius <= max_search_cells; ++radius) {
    ...
    for (int dx = -radius; dx <= radius; ++dx) {
      for (int dy = -radius; dy <= radius; ++dy) {
        ...
        double check_x = goal.pose.position.x + dx * resolution;
        double check_y = goal.pose.position.y + dy * resolution;
        ...
        if (cost < nav2_costmap_2d::INSCRIBED_INFLATED_OBSTACLE) {
          ...
          if (dist < best_dist) {
            best_dist = dist;
            best_point.pose.position.x = check_x;
            best_point.pose.position.y = check_y;
            best_point.pose.position.z = 0.0;
            best_point.pose.orientation = goal.pose.orientation;
            best_point.header = goal.header;
          }
        }
      }
    }
...
nav_msgs::msg::Path
GoalAwareNavfnPlanner::createPlanToReachableGoal(
    const geometry_msgs::msg::PoseStamped &start,
    const geometry_msgs::msg::PoseStamped &adjusted_goal) {
  // Try known planner first
  nav_msgs::msg::Path known_path;
  try {
    known_path = known_planner_->createPlan(start, adjusted_goal);
  } catch (const std::exception &error) {
    RCLCPP_WARN(node_->get_logger(),
                "%s known-only plan to adjusted goal failed: %s",
                name_.c_str(), error.what());
  }
  if (!known_path.poses.empty()) {
    return known_path;
  ...
  // Try unknown planner as fallback
  auto fallback_path = unknown_planner_->createPlan(start, adjusted_goal);
  if (fallback_path.poses.empty()) {
    return fallback_path;
  }
  ...
  if (!withinUnknownBudget(exposure, max_unknown_ratio_, max_unknown_length_,
    ...
    RCLCPP_ERROR(
        node_->get_logger(),
        "%s rejected path to adjusted goal: unknown %.2f/%.2f m (%.1f%%)",
        name_.c_str(), exposure.unknown_length, exposure.path_length,
        ratio * 100.0);
    return nav_msgs::msg::Path();
  }
  ...
  RCLCPP_WARN(node_->get_logger(),
              "%s accepted path to adjusted goal: unknown %.2f/%.2f m (%.1f%%)",
              name_.c_str(), exposure.unknown_length, exposure.path_length,
              ratio * 100.0);
  return fallback_path;
}
