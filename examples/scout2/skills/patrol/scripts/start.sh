...
# patrol skill start (native). Runs the atlas bridge under the conda env,
...
if [ -z "${CONDA:-}" ]; then
    echo "[patrol] error: could not find conda binary."
    exit 1
fi
...
if [ -z "${CONDA_PY:-}" ]; then
    echo "[patrol] error: failed to get Python from conda env '$CONDA_ENV'."
    exit 1
fi
...
exec "$CONDA_PY" -u -m patrol_skill.atlas_bridge
