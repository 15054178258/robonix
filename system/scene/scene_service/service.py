...
async def _start_ros_ingest(
    *,
    ...
    same_class_centroid_max_m = float(
        association_cfg.get("same_class_centroid_max_m", 0.15)
    )
    same_class_min_voxel_coverage = float(
        association_cfg.get("same_class_min_voxel_coverage", 0.50)
    )
    same_class_max_extent_ratio = float(
        association_cfg.get("same_class_max_extent_ratio", 1.75)
    )
    same_class_disjoint_min_unique_frames = int(
        association_cfg.get(
            "same_class_disjoint_min_unique_frames",
            2,
        )
    )
    same_class_disjoint_max_frame_gap = int(
        association_cfg.get(
            "same_class_disjoint_max_frame_gap",
            1,
        )
    )
    same_class_disjoint_max_center_major_extent_ratio = float(
        association_cfg.get(
            "same_class_disjoint_max_center_major_extent_ratio",
            0.20,
        )
    )
    same_class_disjoint_min_visual_similarity = float(
        association_cfg.get(
            "same_class_disjoint_min_visual_similarity",
            0.85,
        )
    )
    same_class_merge_interval_ticks = int(
        association_cfg.get("same_class_merge_interval_ticks", 1)
    )
    ...
    max_detections = int(perception_cfg.get("max_detections", 30))
    ...
    rebase_map_corrections = _geometry_bool(
    ...
    if same_class_centroid_max_m < 0.0:
        raise ValueError(
            "perception.association.same_class_centroid_max_m "
            "must not be negative"
        )
    if not 0.0 <= same_class_min_voxel_coverage <= 1.0:
        raise ValueError(
            "perception.association.same_class_min_voxel_coverage "
            "must be in [0, 1]"
        )
    if same_class_max_extent_ratio < 1.0:
        raise ValueError(
            "perception.association.same_class_max_extent_ratio "
            "must be at least one"
        )
    if same_class_disjoint_min_unique_frames < 1:
        raise ValueError(
            "perception.association."
            "same_class_disjoint_min_unique_frames must be at least one"
        )
    if same_class_disjoint_max_frame_gap < 0:
        raise ValueError(
            "perception.association."
            "same_class_disjoint_max_frame_gap must not be negative"
        )
    if same_class_disjoint_max_center_major_extent_ratio < 0.0:
        raise ValueError(
            "perception.association."
            "same_class_disjoint_max_center_major_extent_ratio "
            "must not be negative"
        )
    if not -1.0 <= same_class_disjoint_min_visual_similarity <= 1.0:
        raise ValueError(
            "perception.association."
            "same_class_disjoint_min_visual_similarity must be in [-1, 1]"
        )
    if same_class_merge_interval_ticks < 1:
        raise ValueError(
            "perception.association.same_class_merge_interval_ticks "
            "must be at least one"
        )
    ...
    if plan.detector == "concept_graphs":
        ...
        detector = ConceptGraphsDetector(
            ...
            rebase_map_corrections=rebase_map_corrections,
            ...
            same_class_centroid_max_m=same_class_centroid_max_m,
            same_class_min_voxel_coverage=(
                same_class_min_voxel_coverage
            ),
            same_class_max_extent_ratio=same_class_max_extent_ratio,
            same_class_disjoint_min_unique_frames=(
                same_class_disjoint_min_unique_frames
            ),
            same_class_disjoint_max_frame_gap=(
...
async def _run_active(config: dict) -> None:
    ...
    if broadcast is not None and not str(broadcast.get("map_id") or ""):
        ...
        log.warning(
            ...
            "while scene binds %r from %s — mapping mode is using an unsaved "
            "live session; Save the current mapping session as %r first, "
            "then Load that saved map or restart in localization mode for a "
            "stable cross-boot binding",
            binding.map_id,
            binding.source,
            binding.map_id,
        )
    ...
    bg_tasks = [
        ...
        # Preserve dev's object-level memory bridge while Scene owns the
        # richer lifecycle below.  It is deliberately independent of the
        # lifecycle watcher so disabling it cannot affect Scene health.
        *(
            [
                asyncio.create_task(
            ...
            if os.environ.get("SCENE_OBJECT_WATCHDOG", "1")
            in ("1", "true", "yes")
            else []
        ),
