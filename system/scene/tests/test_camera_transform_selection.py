...
class _FakeHub:
    ...
    def lookup_transform_4x4(
        ...
        self.lookup_calls.append((target_frame, source_frame, stamp))
...
def _detector(
    ...
    *,
    ...
    detector._rebase_map_corrections = rebase_map_corrections
    return detector
...
def test_tf_transform_wins_when_explicit_extrinsics_are_also_available() -> None:
    ...
    assert hub.lookup_calls == [("selected_camera_optical", "map", None)]
...
def test_tf_transform_uses_the_observation_timestamp() -> None:
    stamp = SimpleNamespace(sec=123, nanosec=456)
    hub = _FakeHub(np.eye(4, dtype=np.float64), {})

    actual = _detector(hub)._build_camera_to_map_transform(stamp=stamp)

    assert actual is not None
    assert hub.lookup_calls == [
        ("selected_camera_optical", "map", stamp),
    ]


def test_stamped_tf_falls_back_to_exact_odom_plus_latest_map_correction() -> None:
    stamp = SimpleNamespace(sec=123, nanosec=456)
    odom = SimpleNamespace(header=SimpleNamespace(frame_id="odom"))
    camera_to_odom = np.eye(4, dtype=np.float64)
    camera_to_odom[:3, 3] = [1.0, 0.0, 0.0]
    odom_to_map = np.eye(4, dtype=np.float64)
    odom_to_map[:3, 3] = [0.0, 2.0, 0.0]

    class _SplitTfHub(_FakeHub):
        def lookup_transform_4x4(
            self,
            target_frame: str,
            source_frame: str,
            ...
            self.lookup_calls.append((target_frame, source_frame, stamp))
            return {
                ("selected_camera_optical", "map"): None,
                ("selected_camera_optical", "odom"): camera_to_odom,
                ("odom", "map"): odom_to_map,
            }[(target_frame, source_frame)]

    hub = _SplitTfHub(None, {"odom": odom})
    detector = _detector(hub)
    actual = detector._build_camera_to_map_transform(stamp=stamp)

    expected = np.eye(4, dtype=np.float32)
    expected[:3, 3] = [1.0, 2.0, 0.0]
    np.testing.assert_allclose(actual, expected)
    assert hub.lookup_calls == [
        ("selected_camera_optical", "odom", stamp),
        ("odom", "map", None),
    ]
    assert (
        detector._last_transform_source
        == "stamped_odom_plus_current_map_correction"
    )


class _TransformableGeometry:
    """Minimal Open3D-like geometry for map-correction tests."""
    ...
    def transform(self, matrix) -> None:
        ...
        homogeneous = np.column_stack(
            [self.points, np.ones(len(self.points), dtype=np.float64)]
        )
        self.points = (transform @ homogeneous.T).T[:, :3]
        self.transforms.append(transform.copy())


def test_map_correction_keeps_existing_map_geometry_fixed_by_default() -> None:
    detector = _detector(_FakeHub(None, {}))
    pcd = _TransformableGeometry([[1.0, 2.0, 0.5]])
    bbox = _TransformableGeometry([[1.0, 2.0, 0.5]])
    detector._map_objects = [{"pcd": pcd, "bbox": bbox}]
    detector._last_world_from_odom = np.eye(4, dtype=np.float64)
    correction = np.eye(4, dtype=np.float64)
    correction[:3, 3] = [0.4, -0.2, 0.0]

    assert detector._adopt_world_from_odom_correction(correction)

    np.testing.assert_allclose(pcd.points[0], [1.0, 2.0, 0.5])
    np.testing.assert_allclose(bbox.points[0], [1.0, 2.0, 0.5])
    assert detector._quality_counters["map_correction_rebases"] == 0


def test_map_correction_rebases_existing_conceptgraphs_geometry() -> None:
    detector = _detector(
        _FakeHub(None, {}),
        rebase_map_corrections=True,
    )
    pcd = _TransformableGeometry([[1.0, 2.0, 0.5]])
    bbox = _TransformableGeometry([[1.0, 2.0, 0.5]])
    detector._map_objects = [{"pcd": pcd, "bbox": bbox}]
    ...
    np.testing.assert_allclose(pcd.points[0], [1.4, 1.8, 0.5])
    np.testing.assert_allclose(bbox.points[0], [1.4, 1.8, 0.5])
    assert detector._quality_counters["map_correction_rebases"] == 1
    assert detector._map_correction_total_translation_m == pytest.approx(
        math.hypot(0.4, 0.2)
    )


def test_map_correction_rebase_uses_incremental_delta() -> None:
    detector = _detector(
        _FakeHub(None, {}),
        rebase_map_corrections=True,
    )
    pcd = _TransformableGeometry([[0.0, 0.0, 0.0]])
    bbox = _TransformableGeometry([[0.0, 0.0, 0.0]])
    detector._map_objects = [{"pcd": pcd, "bbox": bbox}]
    detector._last_world_from_odom = np.eye(4, dtype=np.float64)
    ...
    assert detector._adopt_world_from_odom_correction(first)
    assert detector._adopt_world_from_odom_correction(second)

    np.testing.assert_allclose(pcd.points[0], [0.5, 0.0, 0.0])
    assert detector._quality_counters["map_correction_rebases"] == 2
