...
class _Detector:
    ...
    def export_3d_snapshot(self, *, include_clip_feature=False):
        self.snapshot_requests.append(bool(include_clip_feature))
        return {"objects": [], "stamp_unix": 1.0}
...
def test_web_object_correction_returns_typed_http_errors_and_updates() -> None:
    ...
    coordinator, registry, derived, _, detector, *_ = _coordinator()
    app = web.make_app(
        registry=registry,
        detector=detector,
    ...
    async def exercise():
        ...
        assert detector.snapshot_requests == [False]

        status, out = await _web_request(
            app,
            "GET",
            "/api/objects3d?debug_clip_features=1",
        )
        assert status == 200
        assert detector.snapshot_requests == [False, True]
