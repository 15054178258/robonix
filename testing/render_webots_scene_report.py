...
canvas{display:block;width:100%;height:380px;background:var(--paper)}
.orbit{height:clamp(300px,42vh,420px);min-height:0}
...
.spatial{height:clamp(300px,40vh,400px)}
...
@media(max-width:700px){body{margin:12px}.summary-head{display:block}.result{display:block;margin-top:5px}canvas,.orbit,.spatial{height:320px}.viewer-toolbar{flex-wrap:wrap}.viewer-toolbar strong{width:100%}}
</style>
</head>
<body>
...
function enableOrbit(viewer,world){
  // Open on the complete room/map extent. Reviewers need the global spatial
  // context first; close inspection is an explicit wheel, toolbar, or
  // Focus-selected action.
  // zoom=1 already means that the active frame fills the viewport.  Start
  // deliberately closer for bbox inspection while retaining "Fit all objects"
  // and "Fit room" as explicit overview actions.  The zoom ceiling is
  // effectively unbounded, and both the wheel and toolbar button cover enough
  // range to inspect a small bbox without dozens of inputs.
  const closeZoom=10,focusZoom=12,fitObjectZoom=1.0,roomZoom=.96;
  const canvas=viewer.querySelector("canvas"),v={yaw:-.75,pitch:.65,zoom:roomZoom,minZoom:.03,maxZoom:1e18,zoomStep:3.0,panX:0,panY:0,target:null,focusRadius:null,frameMode:"room",labelMode:"hover",selected:null,hovered:null,hits:[]};
  let drag=false,panning=false,moved=false,wheelActive=false,last=[0,0],selectItem;
  ...
  canvas.onpointerdown=e=>{wheelActive=true;drag=true;panning=e.button===2||e.shiftKey;moved=false;last=[e.clientX,e.clientY];canvas.setPointerCapture(e.pointerId)};
  ...
  canvas.onpointerleave=()=>{wheelActive=false;if(!drag&&v.hovered){v.hovered=null;render()}};
  canvas.ondblclick=e=>{const item=nearestHit(v.hits,e.offsetX,e.offsetY,32);if(item){selectItem(item);focusItem(item)}};
  canvas.onwheel=e=>{if(!wheelActive)return;e.preventDefault();const old=v.zoom,deviceDelta=e.deltaY*(e.deltaMode===1?24:e.deltaMode===2?canvas.clientHeight:1),wheelDelta=Math.max(-400,Math.min(400,deviceDelta)),next=Math.max(v.minZoom,Math.min(v.maxZoom,old*Math.exp(-wheelDelta*.016))),ratio=next/old;
...
function enableSpatial(viewer,world,visibility){
  const canvas=viewer.querySelector("canvas"),v={zoom:1.0,minZoom:.05,maxZoom:1e9,zoomStep:2.2,panX:0,panY:0,target:null,labelMode:"hover",selected:null,hovered:null,hits:[]};
  const images={};let paint=()=>{},drag=false,moved=false,wheelActive=false,last=[0,0],selectItem;
  ...
  canvas.onpointerdown=e=>{wheelActive=true;drag=true;moved=false;last=[e.clientX,e.clientY];canvas.setPointerCapture(e.pointerId)};
  canvas.onpointermove=e=>{if(!drag){const hovered=nearestHit(v.hits,e.offsetX,e.offsetY,18);
      if((hovered?.box.id||null)!==(v.hovered?.box.id||null)||(hovered?.source||null)!==(v.hovered?.source||null)){v.hovered=hovered;render()}return}
  ...
  canvas.onpointerleave=()=>{wheelActive=false;if(!drag&&v.hovered){v.hovered=null;render()}};
  canvas.ondblclick=e=>{const item=nearestHit(v.hits,e.offsetX,e.offsetY,32);if(item){selectItem(item);focusItem(item)}};
  canvas.onwheel=e=>{if(!wheelActive)return;e.preventDefault();const old=v.zoom,next=Math.max(v.minZoom,Math.min(v.maxZoom,old*Math.exp(-e.deltaY*.02))),ratio=next/old;
...
function renderSummary(){
  ...
  const headers=["World","Coverage","Path m","Wall ≤10 cm","Wall P95 m","Angle P95°","TP","FP","FN","Candidates","Precision","Recall","F1","Label GPT / raw","Duplicate","Ghost","Center m","Cloud inlier","Gate"];
  ...
  const rows=DATA.map(world=>{const m=world.metrics||{},invalid=world.benchmark_status==="invalid",ok=!invalid&&Boolean(m.ok);
    ...
    return `<tr><td>${world.world}</td><td>${pct(world.coverage)}</td><td>${metres(world.path_length_m)}</td><td>${pct(map.observed_wall_coverage_0_10)}</td><td>${metres(map.observed_wall_p95_error_m)}</td><td>${Number.isFinite(map.wall_angle_p95_error_deg)?map.wall_angle_p95_error_deg.toFixed(2):"—"}</td><td>${m.tp??"—"}</td><td>${m.fp??"—"}</td><td>${m.fn??"—"}</td><td>${world.withheld_candidate_count??0}</td><td>${pct(m.precision)}</td><td>${pct(m.recall)}</td><td>${pct(m.f1)}</td><td>${label}</td><td>${pct(m.duplicate_rate)}</td><td>${pct(m.ghost_rate)}</td><td>${metres(m.median_center_error_m)}</td><td>${pct(world.point_cloud_inlier_fraction)}</td><td class="${ok?"pass":"fail"}"${title}>${gate}</td></tr>`;
  }).join("");
...
DATA.forEach(world=>{const section=document.createElement("section"),m=world.metrics;
  ...
  section.innerHTML=`<h2>${world.world}</h2><div class="metrics">coverage ${(100*(world.coverage||0)).toFixed(1)}% · path ${(world.path_length_m||0).toFixed(2)} m · walls ≤10 cm ${pct(map.observed_wall_coverage_0_10)} · wall P95 ${metres(map.observed_wall_p95_error_m)} m · angle P95 ${Number.isFinite(map.wall_angle_p95_error_deg)?map.wall_angle_p95_error_deg.toFixed(2):"—"}° · TP ${m.tp} · FP ${m.fp} · FN ${m.fn} · candidates ${world.withheld_candidate_count??0} · F1 ${(m.f1||0).toFixed(3)} · ${labelText} · ${snap}</div>${coverageNote}<div class="hint">Solid boxes = confirmed Scene objects; blue boxes = withheld raw candidates; dashed boxes = WBT truth. Candidates remain inspectable but are excluded from TP/FP/FN. Click the view to enable wheel zoom; drag to rotate; Shift-drag or right-drag to pan; moving outside restores page scrolling. Double-click or use Focus selected for a close view.</div>${viewerMarkup("orbit","Perspective 3D")}<div class="hint">The same boxes in the live SLAM map frame. Click the view to enable wheel zoom; drag to pan; moving outside restores page scrolling. Use + / − or double-click a box to focus.</div>${viewerMarkup("spatial","Spatial map")}`;
