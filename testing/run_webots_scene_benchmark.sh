...
Environment:
  ...
  ROBONIX_SCENE_BENCHMARK_MOTION_MODE (reactive | explore)
...
if [[ "$MOTION_MODE" != "reactive" && "$MOTION_MODE" != "explore" ]]; then
...
for world in "${WORLDS[@]}"; do
    ...
    python3 - "$world_dir/startup-state.json" "$MANIFEST" <<'PY'
import json
import math
import sys
...
expected_rebase_map_corrections = bool(
    geometry.get("rebase_map_corrections", True)
)
...
PY
    ...
    sweep_command="source /opt/ros/humble/setup.bash && python3 /tmp/run_webots_scene_sweep.py \
      ...
      --alignment-timeout-s ${ROBONIX_SCENE_ALIGNMENT_TIMEOUT_S:-45} \
...
PY
    ...
    # Give the last RGB-D observations one cleanup interval to reach Scene.
    sleep 8
