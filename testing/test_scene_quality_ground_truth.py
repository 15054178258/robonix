...
class SceneQualityGroundTruthTests(unittest.TestCase):
    ...
    def test_axis_angle_yaw_uses_xy_projection_not_axis_shape(self) -> None:
        self.assertAlmostEqual(_planar_yaw((0.0, 0.0, 1.0, 0.75)), 0.75)
        self.assertAlmostEqual(_planar_yaw((0.0, 0.0, -1.0, 0.75)), -0.75)
        # The apartment robot is serialized with a tiny non-planar component.
        # Its projected heading is still well-defined and must not make the
        # cross-world benchmark fail before Scene is even evaluated.
        yaw = _planar_yaw((1.0, 0.0, 0.0053525, 0.0053525))
...
class MultiObjectSceneGroundTruthTests(unittest.TestCase):
    ...
    def test_matching_scales_to_cross_world_inventory(self) -> None:
        truths = tuple(
            ObjectGroundTruth(
                f"fixture-{index}",
                (float(index) * 2.0, 0.0, 0.5),
                (0.5, 0.5, 1.0),
                0.0,
                0.4,
            )
        ...
        matches = match_objects_one_to_one(objects, truths)
        self.assertEqual(len(matches), 70)
        self.assertEqual(
            [match.obj["id"] for match in matches],
            [f"object-{index}" for index in range(70)],
        )
